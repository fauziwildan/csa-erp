...
<div class="flex h-full" x-data="{ sidebarOpen: {{ $collapseSidebar ? 'false' : 'window.innerWidth >= 1024' }} }">
    ...
    >
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-0.5 text-sm">
            ...
            @can('reset stock')
            <a href="{{ route('stock-reset.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-lg font-medium transition-colors {{ request()->routeIs('stock-reset.*') ? 'bg-red-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
